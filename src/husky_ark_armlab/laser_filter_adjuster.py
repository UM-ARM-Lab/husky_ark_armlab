...
class LaserFilterAdjuster:
    def __init__(self):
        self.front_client = dynamic_reconfigure.client.Client(
            "/laser_filter_front/range/"
        )
        self.rear_client = dynamic_reconfigure.client.Client(
            "/laser_filter_rear/range/"
        )
    ...
    def set_radius(self, amount):
        ...
        if (
            self.front_upper_threshold() == amount
            and self.rear_upper_threshold() == amount
        ):
            return
